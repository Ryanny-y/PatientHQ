...
import { HeartPulse, User, Phone, Mail, Calendar, Clock, FileText, Activity, ClipboardList } from 'lucide-react';
import {
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle>{nurse.full_name}</DialogTitle>
          ...
          <div>
            ...
            <div className="space-y-3.5">
              <div className="grid grid-cols-2 gap-3">
                <Field label="Nurse ID" value={`NRS-${String(nurse.nurse_id).padStart(4, '0')}`} />
                <Field label="User ID" value={`USR-${String(nurse.user_id).padStart(4, '0')}`} />
              </div>
              <Field label="Username" value={nurse.username} icon={User} />
              <Field label="Full Name" value={nurse.full_name} icon={User} />
              <Field label="Email Address" value={nurse.email} icon={Mail} />
              <Field label="Contact Number" value={nurse.contact_number} icon={Phone} />
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <p className="text-xs text-slate-400 mb-1">Assigned Ward</p>
                  <WardBadge ward={nurse.assigned_ward} showIcon />
                </div>
                <Field label="License Number" value={nurse.license_number} icon={FileText} />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
              ...
              <Field
                label="Created Date"
                value={new Date(nurse.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
                icon={Calendar}
              />
            </div>
          </div>
