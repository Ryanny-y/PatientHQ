...
interface DoctorResetPasswordModalProps {
      ...
      <DialogContent className="max-w-sm">
        <DialogHeader>
          <div className="flex items-center gap-2">
            ...
            <div>
              <DialogTitle>Reset Physician Password</DialogTitle>
              <DialogDescription>{doctor ? `For @${doctor.username}` : ''}</DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(handleReset)} className="px-6 py-5 space-y-4">
          <div className="p-3 rounded-lg bg-amber-50 border border-amber-200 text-xs text-amber-700">
            The physician will need to use the new password on their next login.
          </div>
